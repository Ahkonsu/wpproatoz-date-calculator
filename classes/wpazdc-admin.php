...
/**
 * WPAZ_DateCalc_Admin Class
 *
 * @class WPAZ_DateCalc_Admin
 * @version	1.0.0
 * @since 1.0.0
 * @package	WPAZ_DateCalc
 * @author dewolfe001
 */
final class WPAZ_DateCalc_Admin {
	/**
	...
	/**
	 * Main WPAZ_DateCalc_Admin Instance
	 *
	 * Ensures only one instance of WPAZ_DateCalc_Admin is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 * @static
	 * @return Main WPAZ_DateCalc_Admin instance
	 */
	...
	public function register_settings_screen () {
		$this->_hook = add_submenu_page( 'options-general.php', __( 'Date Calculator Settings', 'wpazdc' ), __( 'Date Calculator', 'wpazdc' ), 'manage_options', 'wpazdc', array( $this, 'settings_screen' ) );
	...
	public function settings_screen () {
		global $title;
		$sections = WPAZ_DateCalc()->settings->get_settings_sections();
		$tab = $this->_get_current_tab( $sections );
		?>
		<div class="wrap wpazdc-wrap">
			<?php
				echo $this->get_admin_header_html( $sections, $title );
			?>
			<form action="options.php" method="post">
				<?php
					settings_fields( 'wpazdc-settings-' . $tab );
					do_settings_sections( 'wpazdc-' . $tab );
					submit_button( __( 'Save Changes', 'wpazdc' ) );
				?>
			</form>
	...
	public function register_settings () {
		...
		if ( 0 < count( $sections ) ) {
			foreach ( $sections as $k => $v ) {
				register_setting( 'wpazdc-settings-' . sanitize_title_with_dashes( $k ), 'wpazdc-' . $k, array( $this, 'validate_settings' ) );
				add_settings_section( sanitize_title_with_dashes( $k ), $v, array( $this, 'render_settings' ), 'wpazdc-' . $k, $k, $k );
			}
		}
	...
	public function render_settings ( $args ) {
		$token = $args['id'];
		$fields = WPAZ_DateCalc()->settings->get_settings_fields( $token );
		...
		if ( 0 < count( $fields ) ) {
			foreach ( $fields as $k => $v ) {
				...
				add_settings_field( $k, $v['name'], array( WPAZ_DateCalc()->settings, 'render_field' ), 'wpazdc-' . $token , $v['section'], $args );
			}
		}
	} // End render_settings()
	...
	public function validate_settings ( $input ) {
		$sections = WPAZ_DateCalc()->settings->get_settings_sections();
		$tab = $this->_get_current_tab( $sections );
		return WPAZ_DateCalc()->settings->validate_settings( $input, $tab );
	...
	public function get_admin_header_html ( $sections, $title ) {
		$defaults = array(
							'tag' => 'h2',
							'atts' => array( 'class' => 'wpazdc-wrapper' ),
							'content' => $title
						);
	...
	private function _get_admin_header_data ( $sections, $title ) {
		$response = array( 'tag' => 'h2', 'atts' => array( 'class' => 'wpazdc-wrapper' ), 'content' => $title );
		...
		if ( is_array( $sections ) && 1 < count( $sections ) ) {
			...
			foreach ( $sections as $key => $value ) {
				...
				$response['content'] .= '<a href="' . admin_url( 'options-general.php?page=wpazdc&tab=' . sanitize_title_with_dashes( $key ) ) . '" class="' . esc_attr( $class ) . '">' . esc_html( $value ) . '</a>';
			}
		}

		return (array)apply_filters( 'wpazdc-get-admin-header-data', $response );
